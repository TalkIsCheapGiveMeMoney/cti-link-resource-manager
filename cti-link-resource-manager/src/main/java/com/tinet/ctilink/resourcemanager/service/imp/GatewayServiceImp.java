...
public class GatewayServiceImp extends BaseService<Gateway> implements CtiLinkGatewayService {
    ...
    public String getKey(Gateway gateway){
        return String.format(CacheKey.GATEWAY_NAME,gateway.getName());
    }

    public void setCache(Gateway gateway){
        redisService.set(Const.REDIS_DB_CONF_INDEX,getKey(gateway),gateway);

        List<Gateway> list = redisService.getList(Const.REDIS_DB_CONF_INDEX,String.format(CacheKey.GATEWAY),Gateway.class);
        if(list == null){
            list = new ArrayList<Gateway>();
        }
        list.add(gateway);
        redisService.set(Const.REDIS_DB_CONF_INDEX,String.format(CacheKey.GATEWAY),list);
    }
